...
export default function GraphDetails({ dataSet, title, field, scale, useRechart }) {
    ...
    return (<div style={{ width: '100%', height: '90%' }}>
      <ResponsiveContainer>
        ...
        >
          ...
          <Tooltip />
          <Legend />
          <Line type="monotone" name="Cairns" dataKey="Cairns" stroke="#ff0000" dot={false} isAnimationActive={false} />
          <Line type="monotone" name="Gold Coast" dataKey="Gold Coast" stroke="#00ff00" dot={false} isAnimationActive={false} />
          <Line type="monotone" name="Noosa" dataKey="Noosa" stroke="#0000ff" dot={false} isAnimationActive={false} />
          <Line type="monotone" name="Whitsunday" dataKey="Whitsunday" stroke="#9B870C" dot={false} isAnimationActive={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
    );
    ...
    let options = {
      ...
      data: dataSet.map(sample => {
        return {
          ...
          "noosa_value": sample.lga_name == 'Noosa' ? parseFloat(sample[field]) * scale : null,
          "gold_coast_value": sample.lga_name == 'Gold Coast' ? parseFloat(sample[field]) * scale : null,
          "whitsunday_value": sample.lga_name == 'Whitsunday' ? parseFloat(sample[field]) * scale : null,
          "cairns_value": sample.lga_name == 'Cairns' ? parseFloat(sample[field]) * scale : null
