import { addDaysToDate, daysBetweenDates } from '../../api';
 ...
 export function DateSliderMedium( {originDate, min, max, title, value, setValue}) {
    ...
    return (
      <div>
        <input type="range" min={min} max={max} value={daysBetweenDates(originDate, value)} className="range" onChange={handleDateValueChange} />
        <div>{title}: {value} </div>
      </div>
    );
  }